...
import { FlatList, Image, StyleSheet, ActivityIndicator, TouchableOpacity } from "react-native";
            ...
            <FlatList
                ListHeaderComponent={<>
                    <Box dir="row" px="sm" mt="sm" mb="md">
                        {podcastData.thumbnail && (<Box mr={10}>
                            <Image style={styles.image} source={{ uri: podcastData.thumbnail }} />
                        </Box>)}
                        <Box f={1}>
                            <Text size="large" bold>{podcastData.podcastName}</Text>
                            <Text size="xs" color="gray">{podcastData.artist}</Text>
                            <Text size="xs" color="green">Subscribe</Text>
                        </Box>
                    </Box>
                    <Box px="sm" mb="md" dir="row" align="center">
                        <Box mr={10}>
                            <TouchableOpacity onPress={() => {
                                ...
                                <FeatherIcon name="play" size={28} color={theme.color.greenLight} />
                            </TouchableOpacity>
                        </Box>
                        <Box f={1}>
                            <Text bold>Play</Text>
                    ...
                    <Box px="md" mb="md">
                        <Text size="lg" bold>Episode</Text>
                    </Box>
                    {loading && (
                        <Box h={200} center>
                ...
                renderItem={({ item }) => <Box px="sm">
                    <Text size="sm" color="green">{getWeekDay(new Date(item.pubDate)).toUpperCase()}</Text>
                    <Text bold >{item.title}</Text>
                    <Text size="sm" color="gray" numberOfLines={2}>{item.description}</Text>
                    <Text size="sm" color="gray" bold>{humanDuration(item.duration)}</Text>
                </Box>}
            />
        </Box>
    )
